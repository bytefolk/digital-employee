...
三个岗位，**提交与评审分属不同的人**，用两个 GitHub 身份把「能提」与「能批」在凭据层面分开。**合并由操作者人工执行**，员工负责在 PR 上准备好合并前核查清单。

| 岗位 | 做 | 绝不做 | 身份 | 工具白名单 |
|---|---|---|---|---|
| `issue-triage` | 查重、复现、分流、按模板建 issue | 不改代码、不提 PR、不评审、不合并 | `author` | `Read` `Grep` `Glob` `Bash` |
| `pr-author` | 实现改动、推分支、开 PR、按意见修改 | **不评审、不合并** | `author` | `Read` `Write` `Edit` `Grep` `Glob` `Bash` |
...
若三拨人共用一个身份，那么 `pr-author` 提的每一条 PR，`pr-reviewer` 都点不出 `APPROVED` → `reviewDecision` 永远拿不到 `APPROVED` → **PR 永远无法满足合并前提条件。**

所以最低限度：**评审身份必须独立于作者身份**。

### 两个身份的细粒度 PAT 权限

| 权限 | `author`（提交） | `reviewer`（评审） | 说明 |
|---|---|---|---|
| `Contents` | **Read and write** | **Read only** | 推分支需要写。评审身份不给写 → **结构上合不了** |
| `Pull requests` | Read and write | Read and write | 评审要点批准/请求变更，所以需要 PR 写 |
...
5. 确认没有正在进行的 release freeze 或部署窗口限制；
6. 一律 `--squash`。

**绝对禁止**：`gh pr merge --admin` 或任何管理员绕过；dismiss 他人的 `CHANGES_REQUESTED`；合并 draft；关闭别人仍在推进的 PR。
...
推论：**能推 PR 的岗位不能是被委托的子员工**，只能是直接派单、且拿到操作者写入授予的员工。所以本团队里 `pr-author` 必须**直接派单**；`pr-reviewer` 可以走委托（评审本就只读）。把提交挂到委托链下会静默失败——它拿不到写权限，也不会有明确报错。
...
- **没有 GitHub MCP。** 内置宿主遇到员工 MCP 绑定会直接让回合失败（`qoder.mcp_binding_unsupported`），因此岗位包的 `mcpServers` 都是空的，GitHub 访问走 `Bash` + `gh` CLI。
...
├── organization.v1alpha1.json   # workspace-org.v1（owner = null，平铺三岗位）
├── grant.template.json          # capability-grant.v1，操作者持有，位于包目录之外
├── context/README.md            # 运行时工作目录（运行产物）
└── positions/                   # 三个平级岗位包
    ├── issue-triage/
    │   └── employee.json  SKILL.md  permissions.json  budget.json  schemas/  knowledge/  evals/
    ├── pr-author/
    │   └── employee.json  SKILL.md  permissions.json  budget.json  schemas/  knowledge/  evals/
    └── pr-reviewer/
        └── employee.json  SKILL.md  permissions.json  budget.json  schemas/  knowledge/  evals/
...
**没有新造清单格式**：团队用已有的 `workspace.v1alpha1` + `workspace-org.v1` 表达，这两个契约本来就被引擎消费；三个岗位平铺在 `positions/`，每个 `positions/<id>/` 同时也是一份可独立安装的可移植员工包。
