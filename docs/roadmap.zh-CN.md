...
**待评审设计输入：**[安全、可信与业务本体](trust-ontology.zh-CN.md)（#346 R1）建议按
D0 基线 → D1 本体最小契约 → D2 可信只读闭环 → D3 可控行动基础 → D4 有界写入试点
拆分。D0/D1 可先做设计，D2 与本阶段对齐；D3/D4 必须由独立写入需求和资格证据开门。
这不是新增排期或已接受的依赖，不改变 W1、既有 owner 和 gate。
